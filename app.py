...
if city:
    ...
    if df_current is not None:
        predict_hours = 72
        df = predict_n(df_current, predict_hours)
        # if df is not None:
        # df = df.iloc[-49:]
        ...
        vertical_line_time = df['time'].iloc[-(predict_hours + 1)]
        ...
        for i, p in enumerate(pollutants):
            last_real_value = float(df[p].iloc[-(predict_hours + 1)])
            previous_value = float(df[p].iloc[-(predict_hours + 2)])
